...
be included in the parameter estimation as nescessary. The covariance
...
The full set of model parameters to be estimated for the final mixed effects
...
only the diagonals or other partial structure remains to be
estimated. 
...
\begin{equation}
\texttt{ModelPar}:  ~~ \bs\Theta \rightarrow (\bs\theta, \bs\Sigma, \bs\sigma_\omega,
\bs\Omega) .
\end{equation}
...
\begin{tabular}{llr}
\ref{dose2comp} & Dosing in two-compartment model . . . . . . . . .& \pageref{dose2comp} \\
\ref{isr} & Extraction of insulin secretion rate . . . . . . . . .& \pageref{isr} \\
\end{tabular}

\vspace{5mm}

\noindent This document is written using Sweave, and thus all R-code shown in
the examples below can be extracted into an R-script file by writing
\verb|Stangle("PSM.Rnw")| in R to allow for
further experimentaion with the code. The PSM.Rnw
file is located in /doc in the PSM installation folder. 
...
instead the outcome is loaded from a saved .Rdata file. Changing
the flag to \verb|TRUE| in PSM.Rnw and writing
\verb|Sweave("PSM.Rnw")| in R will generate a new document with
analysis based on new simulated data sets.

<<Redo.flag>>=
Redo = FALSE
@ 

\subsection{Dosing in two-compartment model}
\label{dose2comp}
...
\begin{eqnarray}
  dA_1 &=& [-(CL~V_1^i + CL_d V_1^i)A_1 + CL_dV_2A_2]dt + \sigma_1d\omega \\
  dA_2 &=& [ CL_dV_1^iA_1 - CL_dV_2A_2]dt - \sigma_1d\omega\\
  Y &=& A_1/V_1^i + e
\end{eqnarray}
...
\begin{eqnarray}
  d\bs A_t &=& \left[ \begin{array}{cc}
      -(CL~V_1^i + CL_d V_1^i) & CL_dV_2  \\
    CL_dV_1^i & - CL_dV_2 \end{array} \right]\bs A_tdt + \left[ 
...
$A_1/V_1^i$ is the measured concentration. It is seens that the
...
Model.SimDose <- list(
                      ...
                      ModelPar = function(THETA){
                        list(theta=list(sig1=THETA[1], S=THETA[2]),
...
Only three parameters in the model will be estimated, as it can be
seen from the \verb|ModelPar| function above. The parameters to be estimated
are $\bs\Theta$ = (\verb|sig1|, \verb|S|, \verb|OMEGA|).

For this example two individuals will be simulated. They will both be
sampled every $10min$ for $400min$ which is described as below.

<<SamplingScheme>>=
SimDose.Data <- vector(mode="list",length=2)
for (i in 1:2) 
...
<<Dose.SimulateData>>=
if(Redo) {
  SimDose.Data <- PSM.simulate(Model.SimDose, SimDose.Data, SimDose.THETA, dt=.1 , individuals=2)
} else 
  load("simdose.RData")
@ 

The simulated data are shown in Figure \ref{fig:fig1}. The first row
shows the observations for the two individuals and the next two show
...
are used and the bounds are $\pm$ a factor 50.
...
true values of $\bs\Theta = (10, 20, 0.5)$ and the 95\% conficdence
intervals include the true values. In particular the first parameter
$\sigma_1$ is significant which shows that deviations from a normal
ODE two-compartment model are significant. This could also be tested
...
shows that the system noise and observation noise has been separeted
...
for(id in 1:2) {
  ...
  legend(400, y = max(SimDose.Data[[id]]$Y), legend=c('Smooth est.'),
         lty=c('71A1'),col='blue', xjust=1,yjust=1,bty="n")
  lines(out[[id]]$Time , out[[id]]$Xs[1,]/(V1*exp(out[[id]]$eta)),lty='71A1',col='blue')
  for(i in 1:2) {
    plot(SimDose.Data[[id]]$longTime , SimDose.Data[[id]]$longX[i,],type="l",
         ylab=paste('State',i))
...
\subsection{Extraction of insulin secretion rate}
\label{isr}

Insulin secretion rates (ISR) can be estimated based on measurements of the
concentration of C-peptide in the blood, since insulin an C-peptide
are secreted in equi-molar amounts. This example will first
illustrate a way to simulate C-peptide data based on a model for the ISR,
and then how ISR can be estimated again using a more simple model. The
models used are described in further detail in \cite{klim06}.

The simulated measurements of C-peptide will span over 24H, during
which the patients will recieve three meals at 8 a.m., 12 a.m. and 6
...
The first two states of the simulation model is concentraion in compartment 1 and
2. The third $ISR$ is the secretion which is modelled as a structural part based
on the three meal time plus a constant baseline $B$ and random noise
through a Wiener process. The fourth state $Q$ is used to model
ISR, where $Q$ is controlled by an input which is equal to 1 for
$30min$ af meal times. The model can be defined as
...
The model is definded in PSM as:
...
Everything is now setup and the simulation can be
performed. PARAMETERVAERDIER... HVORFRA ETC.

<<ISR.SimulateData>>=
Sim.THETA <-  c(0.02798 , 0.01048 , 4 , 427.63 , 1.7434)
if(Redo) {
  Sim.Data <- PSM.simulate(Model.SimISR, Sim.Data, Sim.THETA, dt=.1 ,individuals=2) 
} else 
  load("simisr.RData")
@ 

The simulated data are shown in Figure \ref{fig:fig3}.
...
SOMETHING ABOUT ETAS.

ESTIMATION MODEL - (could be simplified by removing U from data.)
...
Do the estimation - choose model, data...!
...
Look at CI. Do smoothing. Estimation time (aalso dose model).
...
Smooth states. Figure \ref{fig:fig4}
...
Plot ISR with bounds for an individual. Figure \ref{fig:fig5}.
\begin{figure}[htb]
\begin{center}
<<label=figure5,fig=TRUE,echo=FALSE,height=3.5,width=6>>=
...
\begin{thebibliography}{99}
  ...
  \bibitem{klim06}
    ...
    Denmark. \\ 
    \footnotesize{http://www2.imm.dtu.dk/pubdb/views/edoc\_download.php/4533/pdf/imm4533.pdf}
  \bibitem{vancauter92}
