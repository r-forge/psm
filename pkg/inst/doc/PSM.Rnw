...
\item \verb+PSM.plot(Data, Smooth, indiv, type)+\\
  ...
  can show observations, simulated and estimated states, simulated or
  estimated eta-values, residuals,
  auto-correlation functions, the x and y axis can be on log-scale and
  it is possible to list estimated random effects on the plot.
\item \verb+PSM.template(Linear,dimX,dimY,dimU,dimEta,file=""))+\\
  *** add numerical eta option in the function ***
