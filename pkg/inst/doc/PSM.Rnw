...
stage. Some of the most essential parts of the implentation, namely
the Kalman filter, is run using compiled code written in Fortran,
...
\noindent This document is written using
Sweave\footnote{http://www.ci.tuwien.ac.at/$\sim$leisch/Sweave/}, and thus
all R-code shown in the examples can be extracted into an R-script
file by writing \verb|Stangle("PSM.Rnw")| in R. This allows for easy
access to further experimentaion with the code behind the examples.
The PSM.Rnw file is located in /doc in the PSM installation folder.
...
<<Dose.SimulateData>>=
if(Redo) {
  SimDose.Data <- PSM.simulate(Model.SimDose, SimDose.Data, SimDose.THETA, deltaTime=.1 , individuals=2)
} else 
  load("simdose.RData")
@ 
...
can be definded in PSM as follows.
...
if(Redo) {
  Sim.Data <- PSM.simulate(Model.SimISR, Sim.Data, Sim.THETA, deltaTime=.1 ,individuals=2) 
} else 
  load("simisr.RData")
@ 
...
simulated values for $\bs\Theta$ = \verb|Sim.THETA| = (\Sexpr{Sim.THETA[1]}, \Sexpr{Sim.THETA[2]},
\Sexpr{Sim.THETA[3]}, \Sexpr{Sim.THETA[4]}) are nicely contained
within the limits.

\large
\textcolor{blue}{\textbf{Discuss estimation times.}} (Remove usepackage(color)).
\normalsize
