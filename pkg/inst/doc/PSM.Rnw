...
This document is written using Sweave, and thus all R-code shown in
the examples below can be extracted
into an R-script file by writing \verb|Stangle("PSM.Rnw")| in R. The .Rnw
file is located in /doc in the PSM installation folder. To save
time in the processing of the document all computer intensive
operations are skipped by setting a flag \verb|Redo = FALSE| and
instead the outcome are loaded from an .Rdata file. If the flag is
changed to \verb|TRUE| in the extracted R-script then all calculation
are re-done. 
...
\right]d\bs\omega\\ 
  Y_{ij} &=& [ \begin{array}{cc} 1/V_1 & 0 \end{array}]\bs A_{ij} + e_{ij}
\end{eqnarray}
