...
In the evaluation of the non-linear model is nescessary to specify a
...
estimated are donoted $\bs \Theta$ and the observed information is
then defined as
...
which is equal to the Hessian matrix of the negative
...
This is used in PSM to provied a Wald 95\% confidence interval,
...
The function \verb+PSM.template+ helps to setup a new a model by
making a model template script.

Review the examples in the vignette. The R-code from the examples in
the vignette is availble on the PSM homepage.
