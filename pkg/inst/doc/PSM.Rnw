...
the $i$th individual. This likelihhod function is evaluated using the
...
states. Confidence bands at a timepoint $t$ are directly given by the
...
The data object contains sample times, observatinos and possible input
and covariates for all individuals and the model object contains
...
<<PSMtemplateLinear>>=
PSM.template(Linear=TRUE,dim=3,dimY=1,dimU=0,dimEta=3)
@ 
...
Input arg. & Type \\
\hline 
\verb+THETA+ &  Vector (possibly named) containg the parameters to be
...
data set. Each element in the list contains
...
<<DataObj>>=
MyData = list( list(Time=1:4,Y=matrix(c(2.1,3.2,3.4,3.7),nrow=1),covar=c(BMI=20.1)), list(Time=1:5,Y=matrix(c(1.9,2.1,2.0,2.9,3.5),nrow=1),covar=c(BMI=23.4)))
@ 

\subsubsection*{Main functions}

PSM is build around three main functions:

\begin{itemize}
\item \verb+PSM.simulate+
\item \verb+PSM.estimate+
\item \verb+PSM.smooth+
\end{itemize}


%Par object PSM.est - good idea to give initial estimates.

%does not have to be sampled at the
%same time... 


%Please also refer to the help files in R.
